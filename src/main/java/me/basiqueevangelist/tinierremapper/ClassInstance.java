...
package me.basiqueevangelist.tinierremapper;
...
import me.basiqueevangelist.tinierremapper.api.TrField;
import me.basiqueevangelist.tinierremapper.api.TrMember;
import me.basiqueevangelist.tinierremapper.api.TrMember.MemberType;
import me.basiqueevangelist.tinierremapper.api.TrMethod;
...
public final class ClassInstance implements TrClass {
	...
	void propagate(MemberType type, String originatingCls, String idSrc, String nameDst,
				   TinyRemapper.Direction dir, boolean isVirtual, boolean fromBridge,
		...
		if (member != null) {
			...
			if (first
				...
				return;
			} else if (tr.propagateBridges != TinyRemapper.LinkedMethodPropagation.DISABLED
					&& member.cls.isInput
					&& isVirtual
				...
				if (bridgeTarget != null) {
					...
					propagate(TrMember.MemberType.METHOD, originatingCls, bridgeTarget.getId(), nameDst, TinyRemapper.Direction.DOWN, true, tr.propagateBridges == TinyRemapper.LinkedMethodPropagation.COMPATIBLE, false, visitedUpBridge, visitedDownBridge);
				}
			}
		} else { // member == null
			...
			for (ClassInstance node : parents) {
				if (visitedUp.add(node)) {
					node.propagate(type, originatingCls, idSrc, nameDst,
							TinyRemapper.Direction.UP, isVirtual, fromBridge,
		...
		if (dir == TinyRemapper.Direction.ANY || dir == TinyRemapper.Direction.DOWN || isVirtual && member != null && (member.access & (Opcodes.ACC_STATIC | Opcodes.ACC_PRIVATE)) == 0) {
			for (ClassInstance node : children) {
				if (visitedDown.add(node)) {
					node.propagate(type, originatingCls, idSrc, nameDst,
							TinyRemapper.Direction.DOWN, isVirtual, fromBridge,
							false, visitedUp, visitedDown);
				}
			}
		}
